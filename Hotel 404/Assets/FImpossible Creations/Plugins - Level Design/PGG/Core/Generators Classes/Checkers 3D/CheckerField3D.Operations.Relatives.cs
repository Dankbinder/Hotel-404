...
namespace FIMSpace.Generating.Checker
{
    public partial class CheckerField3D
    {
        ...
        public bool IsAnyAligning(CheckerField3D other, bool checkY = false)
        {
            ...
            //{
            //    Vector3 wPos = other.GetWorldPos(other.AllCells[o]);
            //    FDebug.DrawBounds2D(new Bounds(wPos, other.RootScale), Color.red, 0.1f);
            //}
            ...
            for (int i = 0; i < AllCells.Count; i++)
            {
                ...
                if (other.ContainsWorld(wPos + RootScaleX, otherIMx)) { return true; }
                if (other.ContainsWorld(wPos - RootScaleX, otherIMx)) { return true; }
                if (other.ContainsWorld(wPos + RootScaleZ, otherIMx)) { return true; }
                if (other.ContainsWorld(wPos - RootScaleZ, otherIMx)) { return true; }
            }
            ...
            if (checkY && YDifferenceExistsBetween(this, other))
            {
                ...
                {
                    Vector3 wPos = GetWorldPos(AllCells[i]);
                    if (other.ContainsWorld(wPos + RootScaleY, otherIMx)) { return true; }
                    if (other.ContainsWorld(wPos - RootScaleY, otherIMx)) { return true; }
                }
            }
        ...
        {
            return new Bounds(MatrixInverse.MultiplyPoint3x4(value.center), value.size);
        }
